...
description: >
  Use when managing secrets, securing the software supply chain, or hardening containers.
...
- **Action**: Run supply chain validation: verify lock files are committed, scan dependencies for
  known vulnerabilities (npm audit/snyk), and check for credential patterns in staged files
...
> See `references/container-security.md` for image security practices, Dockerfile patterns,
> runtime controls, and scanning tools.
...
- Reporting secret exposure risks with file path, secret type, and remediation priority (e.g.,
  "config.js: hardcoded API key pattern detected - move to environment variable")
- Using severity model: CRITICAL = BLOCK (secret in source control, missing lock file, container
  running as root), HIGH = WARN (no pre-commit secret scanning, dependency with known CVE,
  missing SBOM), MEDIUM = INFO (rotation schedule overdue, license compliance gap, image not
  using multi-stage build)
- Providing dependency vulnerability findings with CVE IDs, affected versions, and upgrade paths
- Flagging supply chain attack vectors with SLSA level assessment and specific hardening
  recommendations
