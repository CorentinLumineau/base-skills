...
description: >
  Use when implementing logging, metrics, tracing, or load testing for production
...
- Reporting observability coverage gaps across the three pillars (logs, metrics, traces) with specific missing components per service
- Using severity model: CRITICAL = BLOCK (no structured logging, missing golden signals for production service), HIGH = WARN (no distributed tracing, missing correlation IDs), MEDIUM = INFO (load test baseline gap, dashboard coverage opportunity)
- Providing alerting recommendations with specific threshold values based on the Four Golden Signals and RED/USE methods
- Validating CI integration for load testing against the CI Integration Strategy matrix (PR = smoke, merge = load, pre-release = full)
