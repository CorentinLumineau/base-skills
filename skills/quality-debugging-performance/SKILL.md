---
name: quality-debugging-performance
description: >
  Use when debugging application issues or optimizing performance bottlenecks.
...
- Reporting root cause summaries using the hypothesis-driven format: Observation, Hypothesis, Evidence, Root Cause, Fix
- Using severity model: CRITICAL = BLOCK (band-aid fix instead of root cause, performance regression introduced), HIGH = WARN (N+1 query detected, missing profiling data), MEDIUM = INFO (optimization opportunity, caching candidate)
- Ranking performance bottleneck findings by the Optimization Hierarchy (algorithm > database > cache > micro-optimization)
- Providing before/after measurements for each fix with specific metric targets from the Performance Budget
