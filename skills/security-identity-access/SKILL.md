...
> See `references/compliance-frameworks.md` for SOC 2, GDPR, HIPAA, PCI DSS framework details
> and cross-framework controls.
...
- Reporting identity and access control gaps with component name, control type, and specific
  violation (e.g., "auth middleware: using HS256 instead of RS256 for JWT signing")
- Using severity model: CRITICAL = BLOCK (no authentication on endpoint, default-allow
  authorization), HIGH = WARN (weak password hashing, missing MFA for admin, session fixation
  risk), MEDIUM = INFO (audit logging gap, missing rate limiting on login)
- Validating authorization model against the Pattern Selection guide (RBAC/ABAC/resource-based)
  with coverage per endpoint
- Providing compliance gap assessment against applicable frameworks (SOC 2, GDPR, HIPAA) with
