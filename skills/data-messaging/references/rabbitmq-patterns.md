...
# 'auth.error' matches both bindings above
channel.basic_publish(exchange='logs', routing_key='auth.error', body=body)
```
...
try:
    channel.basic_publish(
        ...
        properties=pika.BasicProperties(
            delivery_mode=2,  # Persistent message (survives restart)
...
def callback(ch, method, properties, body):
    try:
        ...
        # Requeue on transient failure, reject on permanent failure
        ch.basic_nack(
...
channel.basic_consume(
    ...
    auto_ack=False  # Manual acknowledgment — never use auto_ack=True
)
```
...
Messages go to DLX when: consumer rejects with `requeue=False`, message TTL expires, or queue max length exceeded.

## Work Queue (Fair Dispatch)

```python
# Don't give a worker more than 1 unacked message — prevents one slow consumer starving others
...
# High priority
channel.basic_publish(exchange='', routing_key='tasks', body=urgent,
    properties=pika.BasicProperties(priority=9))

# Normal priority
channel.basic_publish(exchange='', routing_key='tasks', body=normal,
...
```python
retry_queues = {
    'retry-1s':  {'x-message-ttl': 1000,  'x-dead-letter-routing-key': 'main-queue'},
    'retry-5s':  {'x-message-ttl': 5000,  'x-dead-letter-routing-key': 'main-queue'},
    'retry-30s': {'x-message-ttl': 30000, 'x-dead-letter-routing-key': 'main-queue'},
}
