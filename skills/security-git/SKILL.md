...
**Hook Integration** — This skill recommends Claude Code hooks:
- **Event**: `PreToolUse` on `Bash`
- **Action**: Trigger pre-commit secret scanning (gitleaks/git-secrets) and verify GPG signing
  is configured before allowing commits
...
**Core principle**: Security controls must be enforced at commit time, before secrets enter git
history.
...
**Critical**: Scan for secrets BEFORE they enter git history. Removing secrets from history is
difficult.
...
Git hooks are executables that run during git operations. Malicious hooks can exfiltrate
code/credentials, modify commits, install backdoors, or tamper with build artifacts.
...
**Server-side best practice**: Verify GPG signatures, reject unsigned commits, scan for secrets
before accepting push, enforce commit message format.
...
- Reporting git security hardening findings with specific control, current state, and required
  action (e.g., "GPG signing: not configured - enable commit.gpgsign=true")
- Using severity model: CRITICAL = BLOCK (secrets detected in staged files, hooks bypassed with
  --no-verify), HIGH = WARN (unsigned commits, missing pre-commit secret scanning), MEDIUM = INFO
  (credential rotation overdue, .gitignore gaps)
- Providing pre-commit hook configuration recommendations with specific tool setup commands
- Flagging supply chain risks from git hooks with threat model assessment (malicious hooks,
